...
parser.add_argument('--beta', default=5.0, type=float, help='beta for out_loss')

parser.add_argument('--epochs', default=100, type=int,
                    help='number of total epochs to run')
parser.add_argument('--ss-epochs', default=100, type=int,
                    help='number of total self-supervised epochs to run')
...
parser.add_argument('--ood-batch-size', default=320, type=int,
                    help='mini-batch size (default: 320)')
...
def main():
    ...
    if args.model_arch == 'densenet':
        model = dn.DenseNet3(args.layers, num_classes + num_reject_classes, args.growth, reduction=args.reduce,
                             bottleneck=args.bottleneck, dropRate=args.droprate, normalizer=normalizer)
    elif args.model_arch == 'wideresnet':
        model = wn.WideResNet(args.depth, num_classes + num_reject_classes, widen_factor=args.width, dropRate=args.droprate, normalizer=normalizer)
    else:
    ...
    for epoch in range(0, args.ss_epochs):
        adjust_learning_rate(optimizer, epoch, lr_schedule)

        # train for one epoch
        train_ss(train_loader, ood_loader, model, criterion, optimizer, epoch, num_classes, num_reject_classes)
...
def train(train_loader, model, criterion, optimizer, epoch, num_classes):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        nat_prec1 = accuracy(nat_output[:, :num_classes].data, target, topk=(1,))[0]
...
def train_ss(train_loader_in, train_loader_out, model, criterion, optimizer, epoch, num_classes, num_reject_classes):
    """Train for one epoch on the training set"""
    batch_time = AverageMeter()
    ...
    for i, (in_set, out_set) in enumerate(zip(train_loader_in, train_loader_out)):
        ...
        random_labels = torch.LongTensor(out_len).random_(num_classes, num_classes + num_reject_classes).cuda()
        out_target = torch.where(pseudo_labels < num_classes, random_labels, pseudo_labels)
        nat_out_loss = criterion(nat_out_output, out_target)

        # measure accuracy and record loss
        nat_prec1 = accuracy(nat_in_output[:, :num_classes].data, target, topk=(1,))[0]
